...
##Evaluation of individual SNP MP
...
short_variant_MP = pd.DataFrame(0, index=u_list_MP, columns = list(sum(zip(pval_id,oddratio_id),())))
...
for sample in id_column:
    ...
    for mp_term in mp_term_sample:
        ...
        gene_in_MP_sample = len(set(u_sample_gene_assoc).intersection(set(mp_term_gene)))
        ##CAlling the fisher test
        short_variant_MP.loc[[mp_term],[sample + '_or']], short_variant_MP.loc[[mp_term],[sample + '_pval']] = stats.fisher_exact([[gene_in_MP_human, gene_in_human], [gene_in_MP_sample, gene_in_sample]])
        
new_list = list(sum(zip(pval_id,oddratio_id),()))
short_variant_MP.columns = new_list
short_variant_MP_merged = short_variant_MP.merge(mp_def,left_index=True,right_on='MP')
...
short_variant_MP_merged.to_sql('short_variant_sample_based_MP', conn, if_exists="replace", index=True)
...
short_variant_MP_merged.to_csv(sys.stdout, sep='\t', index=False)