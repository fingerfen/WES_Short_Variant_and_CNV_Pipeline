...
set_LOF_and_GO_filtered = u_list_GO_gene.map(lambda x : set(x).intersection(set_u_small_gene_list))


for gene_eval_index,gene_eval in enumerate(set_LOF_and_GO_filtered):
...
add_df = pd.concat([GO_df,gene_pval_df,gene_stat_df,gene_yes_sum_df,gene_no_sum_df],axis=1)
...
fin_add_df.to_sql('CNV_GO', conn, if_exists="replace", index=False)
